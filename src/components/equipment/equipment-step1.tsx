"use client"

import { useState, useCallback } from "react"
import { useTRPC } from "@/trpc/client"
import { useQuery, useMutation } from "@tanstack/react-query"
import { Button } from "@/components/ui/button"
import { Input } from "@/components/ui/input"
import { Label } from "@/components/ui/label"
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from "@/components/ui/select"
import {
  Card,
  CardContent,
  CardDescription,
  CardHeader,
  CardTitle,
} from "@/components/ui/card"
import { Separator } from "@/components/ui/separator"
import { ArrowLeft, ArrowRight, Loader2 } from "lucide-react"
import type { EquipmentMeta } from "@/types/equipment"
import { EQUIPMENT_FORMATS } from "@/types/equipment"

type Props = {
  meta: EquipmentMeta
  onDone: (meta: EquipmentMeta, statementId: string) => void
  onBack: () => void
}

export default function EquipmentStep1({ meta, onDone, onBack }: Props) {
  const trpc = useTRPC()
  const [form, setForm] = useState<EquipmentMeta>(meta)
  const [errors, setErrors] = useState<Record<string, string>>({})

  const { data: institutions, isLoading: loadingInst } = useQuery(
    trpc.institution.getAll.queryOptions()
  )
  const { data: branches, isLoading: loadingBranch } = useQuery(
    trpc.branch.getByInstitution.queryOptions(
      { institutionId: form.institutionId },
      { enabled: !!form.institutionId }
    )
  )
  const { data: departmentList, isLoading: loadingDept } = useQuery(
    trpc.department.getByBranch.queryOptions(
      { branchId: form.branchId ?? "" },
      { enabled: !!form.branchId }
    )
  )

  const createStatement = useMutation(trpc.equipment.create.mutationOptions())

  const set = useCallback((key: keyof EquipmentMeta, value: string | null) => {
    setForm((f) => ({ ...f, [key]: value }))
    setErrors((e) => ({ ...e, [key]: "" }))
  }, [])

  function validate() {
    const e: Record<string, string> = {}
    if (!form.institutionId) e.institutionId = "Please select an institution"
    if (!form.program) e.program = "Program is required"
    if (!form.semester) e.semester = "Semester is required"
    if (!form.academicYear) e.academicYear = "Academic year is required"
    if (!form.labWorkshop) e.labWorkshop = "Lab/Workshop name is required"
    return e
  }

  async function handleNext() {
    const e = validate()
    if (Object.keys(e).length > 0) { setErrors(e); return }
    setErrors({})

    const created = await createStatement.mutateAsync({
      institutionId: form.institutionId,
      branchId: form.branchId ?? null,
      departmentId: form.departmentId ?? null,
      program: form.program,
      semester: form.semester,
      academicYear: form.academicYear,
      labWorkshop: form.labWorkshop,
      formatType: form.formatType,
      formNo: form.formNo || null,
      revision: form.revision || null,
      headerDate: form.headerDate || null,
    })

    if (created) onDone(form, created.id)
  }

  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="text-xl">Step 1 — Statement Details</CardTitle>
          <CardDescription>
            Select institution and fill equipment statement details.
          </CardDescription>
        </CardHeader>

        <CardContent className="space-y-6">

          {/* ── Institution ── */}
          <div className="space-y-4">
            <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">Institution</h3>

            <div className="space-y-2">
              <Label>Institution <span className="text-destructive">*</span></Label>
              <Select
                value={form.institutionId}
                onValueChange={(val) => {
                  const inst = institutions?.find((i) => i.id === val)
                  setForm((f) => ({
                    ...f,
                    institutionId: val,
                    institutionName: inst?.name ?? "",
                    institutionCode: inst?.code ?? "",
                    email: inst?.email ?? null,
                    phone: inst?.phone ?? null,
                    logoUrl: inst?.logoUrl ?? null,
                    branchId: null,
                    branchName: null,
                    departmentId: null,
                    departmentName: null,
                    program: "",
                  }))
                  setErrors((e) => ({ ...e, institutionId: "" }))
                }}
              >
                <SelectTrigger className={errors.institutionId ? "border-destructive" : ""}>
                  <SelectValue placeholder={loadingInst ? "Loading..." : "Select institution"} />
                </SelectTrigger>
                <SelectContent>
                  {institutions?.map((inst) => (
                    <SelectItem key={inst.id} value={inst.id}>
                      {inst.name} ({inst.code})
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
              {errors.institutionId && <p className="text-xs text-destructive">{errors.institutionId}</p>}
            </div>

            {form.institutionId && (
              <div className="space-y-2">
                <Label>Branch <span className="text-muted-foreground text-xs">(optional)</span></Label>
                <Select
                  value={form.branchId ?? ""}
                  onValueChange={(val) => {
                    const branch = branches?.find((b) => b.id === val)
                    setForm((f) => ({
                      ...f,
                      branchId: val,
                      branchName: branch?.name ?? null,
                      program: branch?.name ?? f.program,
                      departmentId: null,
                      departmentName: null,
                    }))
                  }}
                >
                  <SelectTrigger>
                    <SelectValue placeholder={loadingBranch ? "Loading..." : "Select branch"} />
                  </SelectTrigger>
                  <SelectContent>
                    {branches?.map((b) => (
                      <SelectItem key={b.id} value={b.id}>{b.name}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
            )}

            {form.branchId && (
              <div className="space-y-2">
                <Label>Department <span className="text-muted-foreground text-xs">(optional)</span></Label>
                <Select
                  value={form.departmentId ?? ""}
                  onValueChange={(val) => {
                    const dept = departmentList?.find((d) => d.id === val)
                    setForm((f) => ({ ...f, departmentId: val, departmentName: dept?.name ?? null }))
                  }}
                >
                  <SelectTrigger>
                    <SelectValue placeholder={loadingDept ? "Loading..." : "Select department"} />
                  </SelectTrigger>
                  <SelectContent>
                    {departmentList?.map((d) => (
                      <SelectItem key={d.id} value={d.id}>{d.name}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
            )}
          </div>

          <Separator />

          {/* ── Format ── */}
          <div className="space-y-4">
            <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">Format</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {EQUIPMENT_FORMATS.map((fmt) => (
                <button
                  key={fmt.id}
                  type="button"
                  onClick={() => set("formatType", fmt.id)}
                  className={`text-left p-4 rounded-lg border-2 transition-all ${
                    form.formatType === fmt.id
                      ? "border-primary bg-primary/5"
                      : "border-border hover:border-muted-foreground/50"
                  }`}
                >
                  <p className="font-semibold text-sm">{fmt.label}</p>
                  <p className="text-xs text-muted-foreground mt-0.5">{fmt.description}</p>
                </button>
              ))}
            </div>
          </div>

          <Separator />

          {/* ── Statement Meta ── */}
          <div className="space-y-4">
            <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">Statement Details</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label>
                  Program <span className="text-destructive">*</span>
                  {form.branchId && (
                    <span className="text-xs text-muted-foreground ml-1">(auto-filled from branch)</span>
                  )}
                </Label>
                <Input
                  placeholder="e.g. Diploma in Computer Science"
                  value={form.program}
                  onChange={(e) => set("program", e.target.value)}
                  className={errors.program ? "border-destructive" : ""}
                />
                {errors.program && <p className="text-xs text-destructive">{errors.program}</p>}
              </div>

              <div className="space-y-2">
                <Label>Semester <span className="text-destructive">*</span></Label>
                <Input
                  placeholder="e.g. 3rd Semester"
                  value={form.semester}
                  onChange={(e) => set("semester", e.target.value)}
                  className={errors.semester ? "border-destructive" : ""}
                />
                {errors.semester && <p className="text-xs text-destructive">{errors.semester}</p>}
              </div>

              <div className="space-y-2">
                <Label>Academic Year <span className="text-destructive">*</span></Label>
                <Input
                  placeholder="e.g. 2024-25"
                  value={form.academicYear}
                  onChange={(e) => set("academicYear", e.target.value)}
                  className={errors.academicYear ? "border-destructive" : ""}
                />
                {errors.academicYear && <p className="text-xs text-destructive">{errors.academicYear}</p>}
              </div>

              <div className="space-y-2">
                <Label>Lab / Workshop <span className="text-destructive">*</span></Label>
                <Input
                  placeholder="e.g. Physics Lab"
                  value={form.labWorkshop}
                  onChange={(e) => set("labWorkshop", e.target.value)}
                  className={errors.labWorkshop ? "border-destructive" : ""}
                />
                {errors.labWorkshop && <p className="text-xs text-destructive">{errors.labWorkshop}</p>}
              </div>
            </div>
          </div>

          <Separator />

          {/* ── Print Header Fields ── */}
          <div className="space-y-4">
            <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">Print Header Fields</h3>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="space-y-2">
                <Label>Form No</Label>
                <Input placeholder="e.g. 001" value={form.formNo} onChange={(e) => set("formNo", e.target.value)} />
              </div>
              <div className="space-y-2">
                <Label>Revision</Label>
                <Input placeholder="e.g. 01" value={form.revision} onChange={(e) => set("revision", e.target.value)} />
              </div>
              <div className="space-y-2">
                <Label>Date</Label>
                <Input type="date" value={form.headerDate} onChange={(e) => set("headerDate", e.target.value)} />
              </div>
            </div>
          </div>

        </CardContent>
      </Card>

      <div className="flex items-center justify-between">
        <Button variant="outline" onClick={onBack} className="gap-2">
          <ArrowLeft className="h-4 w-4" /> Back
        </Button>
        <Button onClick={handleNext} disabled={createStatement.isPending} className="gap-2">
          {createStatement.isPending
            ? <Loader2 className="h-4 w-4 animate-spin" />
            : <ArrowRight className="h-4 w-4" />
          }
          Next — Add Equipment
        </Button>
      </div>
    </div>
  )
}